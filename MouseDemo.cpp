...
int HEIGHT = 544;
int WIDTH = 1080;
int snlBoard1CoordinateX = 894;
int snlBoard1CoordinateY = 0;
...
void diceRunning(){
		...
		if (gotoRight == true)
		{
			...
			snlGuti_X = snlGuti_X + (68 * (rollingValue + 1));
...
void iDraw()
{
	...
	if (dice)
		...
		if (snlGuti_X >= snlBoard1CoordinateX){
			gotoRight = false;
			gotoLeft = false;
...
void iKeyboard(unsigned char key)
{
	if (key == 'r'){
		...
		rollingValue = rand()%6;
		iResumeTimer(rollTimer);
		printf("%d ", rollingValue + 1);
	/*	if (rollingValue >= 1){
			dice_Running = true;
			dice = true;
...
void roll()
{
	...
	if (count > 3)
	{
		if (rollingValue == snlDiceIndex)
		{
			iPauseTimer(rollTimer);
			count = 0;
			diceRunning();
		}
	}
